...
{	
	...
	*/
    ...
    function simpan($nama,$singkatan,$petunjuk)
    {       
        return $this->db->query("insert into model_jawaban (model_id,nama,singkatan,petunjuk) values('NULL','".$nama."','".$singkatan."','".$petunjuk."')");   
    }
	
    function hapus($model_id)
    {
        return $this->db->query( "DELETE  FROM model_jawaban WHERE model_id='".$model_id."'");
    }

    function pilihdata($model_id)
    {
       return $this->db->query( " SELECT * FROM model_jawaban where model_id='".$model_id."'")->row();
    }
