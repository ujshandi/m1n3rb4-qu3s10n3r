...
class Spreadsheet_Excel_Reader {
	 ...
	 */
	function Spreadsheet_Excel_Reader($file='',$store_extended_info=false,$outputEncoding='') {
	 ...
	 * Read the spreadsheet file using OLE, then parse
	 modif chan nambah var error  (ga jadi)
		, & $error
	 */
	...
	function read($sFileName) {
		...
		if($res === false) {
			...
			if($this->_ole->error == 1) {
				...
				//$error = 'The filename ' . $sFileName . ' is not readable';
			//	return;
			}
